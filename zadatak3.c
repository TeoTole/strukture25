...
/* Pomoćna: oslobađanje cijele liste */
...
int main() {
	...
	/* čitanje iz datoteke (prvo očistimo listu) */
...
/* Funkcija za dodavanje elementa na početak liste */
int addToBeginning(Position* head, const char* name, const char* surname, int birthYear) {
	...
	/* Postavljanje novog elementa na početak */
	newPerson->next = *head;
	*head = newPerson;
...
/* Pomoćna funkcija: alocira i postavlja strukturu osobe */
...
/* Pomoćna: oslobađa cijelu listu i postavlja head na NULL */
...
/* A. addAfter - dodaje novi element iza elementa koji ima targetSurname
   Vrati 0 ako uspješno, ELEMENT_NOT_FOUND ako cilj nije pronađen, ili MEMORY_ALLOCATION_FAILED. */
...
/* B. addBefore - dodaje novi element ispred elementa koji ima targetSurname
   Budući da nemamo globalni head, trebamo pokazivač na head (Position*). */
...
int addBefore(Position *head, const char* targetSurname, const char* name, const char* surname, int birthYear) {
	...
	if (curr == NULL) {
		return ELEMENT_NOT_FOUND;/* cilj nije pronađen */
	}
	/* umetanje između prev i curr */
	Position newPerson = createPerson(name, surname, birthYear);
	if (newPerson == NULL) {
		return MEMORY_ALLOCATION_FAILED;
	}
...
int sortBySurname(Position* head) {
	...
	do {
		swapped = 0;
		Position* p = head; // pokazivač NA pokazivač na trenutni čvor
		...
		while ((*p)->next != end) {
			...
			// pomakni pokazivač "p" na sljedeću vezu
			p = &((*p)->next);
		}

		// "end" označava zadnji već sortiran element
		end = *p;
...
/* D. writeToFile - upisuje listu u tekstualnu datoteku, svak jedan zapis po redu:
   ime prezime godina
   Vrati 0 ako je ok, FILE_ERROR ako ne može otvoriti datoteku. */
...
/* E. readFromFile - čita listu iz tekstualne datoteke iste strukture
   Prije čitanja oslobađa postojeću listu da ne bi došlo do curenja memorije.
   Svaka linija: ime prezime godina (razmak razdvaja).
   Vrati 0 ako je ok, FILE_ERROR ako ne može otvoriti datoteku, MEMORY_ALLOCATION_FAILED ako malloc ne uspije. */
